...
import os
import aiohttp
from aiofiles import open as aiofilesOpen  # pip install aiofiles
import asyncio
...
async def requestImgAsync(url):
    try:
        ...
        filename = os.path.basename(filename) + ext.lower()
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as res:
                print(res.headers['Content-Type'])
                async with aiofilesOpen(f'appdata/test/{util.now()}_{filename}', 'wb') as f:
